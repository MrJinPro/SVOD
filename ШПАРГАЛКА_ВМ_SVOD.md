...
Важно про `--host`:
- `--host 127.0.0.1` = доступно ТОЛЬКО на самой ВМ (локально внутри неё).
- `--host 0.0.0.0` = слушает на всех интерфейсах ВМ, можно зайти с другого ПК в локальной сети по `http://<IP_ВМ>:8000`.

Зачем вообще “порты”, если всё внутри компании?
- Любой веб-интерфейс всё равно работает по TCP-порту (например, 8000/5173).
- “Не из интернета” = это про маршрутизацию/фаервол, а не про отсутствие портов.
...
Важно про `--host`:
- `--host 127.0.0.1` = фронт откроется только внутри ВМ.
- `--host 0.0.0.0` = фронт будет доступен с другого ПК в локальной сети по `http://<IP_ВМ>:5173`.
...
Проверка с другого ПК в локальной сети:
- откройте в браузере `http://<IP_ВМ>:8000/docs`
- если видите `ERR_CONNECTION_REFUSED` — обычно backend не запущен/слушает только `127.0.0.1` или порт закрыт фаерволом.
